...
def test_anchor_communities_by_seed_majority():
    ...
    seeds = {"libertarianismo": ["mercado", "privatização"], "comunismo": ["coletivização"]}
    assignment = anchor_communities(communities, seeds)
    assert assignment[0] == "libertarianismo"
    assert assignment[1] == "comunismo"


def test_seed_priority_overrides_community_label():
    # "desregulação" é semente libertarianismo, mas caiu numa comunidade rotulada
    # como social-democracia. A prioridade de semente deve devolvê-la ao
    # libertarianismo e removê-la da social-democracia.
    ...
    seeds = {"libertarianismo": ["desregulação"], "social-democracia": ["bem_estar", "pacto"]}

    term_map = build_ideology_term_map(communities, assignment, centrality, seeds)

    assert "desregulação" in term_map["libertarianismo"]
...
def test_seed_priority_ignores_absent_seeds():
    ...
    communities = [["mercado"]]
    assignment = {0: "libertarianismo"}
    centrality = {"mercado": 1.0}
    seeds = {"libertarianismo": ["mercado"], "comunismo": ["proletariado"]}

    term_map = build_ideology_term_map(communities, assignment, centrality, seeds)

    assert "mercado" in term_map["libertarianismo"]
